      ...
      body: Center(
        child: Column(
          children: [
            ...
            FutureBuilder(
              future: RepositoryProvider.of<ServiceItemsRepository>(
                context,
              ).getItems(),
                      ...
                      return GestureDetector(
                        ...
                        child: ColorFiltered(
                          colorFilter: ColorFilter.mode(
                            Colors.black54,
                            selected == e ? BlendMode.color : BlendMode.dstOver,
                          ),
...
class _MenueditDialogState extends State<MenueditDialog> {
  ...
  TextEditingController cutPercentageController = TextEditingController();
  ...
  void initState() {
    if (widget.data != null) {
      ...
      cutPercentageController.text = widget.data!.cutPercentage.toString();
    }
    super.initState();
  }
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Flexible(
                        child: TextFormField(
                          controller: cutPercentageController,
                          keyboardType: TextInputType.number,
                          validator: (value) => switch (value) {
                            String() when double.tryParse(value) == null =>
                              "Bukan angka valid",
                            String() when value.isEmpty => "Tidak boleh kosong",
                            null => null,
                            String() => null,
                  ...
                  Text(
                    'for employee:${((double.tryParse(cutPercentageController.text) ?? 1) * (int.tryParse(harga.text) ?? 0)).numberFormat(currency: true)}',
                  ),
                  Text(
                    'for owner:${((1 - (double.tryParse(cutPercentageController.text) ?? 1)) * (int.tryParse(harga.text) ?? 0)).numberFormat(currency: true)}',
                  ),
                ],
              ),
            ),
          ...
          onPressed: () async {
            ...
            if (valid) {
              if (selectedimg != null) {
                ...
                // );
                // print(test);
                RepositoryProvider.of<ServiceItemsRepository>(context).addItem(
                  ServiceitemsMdl(
                    title: namaMenu.text,
                    cutPercentage: double.parse(cutPercentageController.text),
