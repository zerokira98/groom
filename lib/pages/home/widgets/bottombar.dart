    ...
    return Stack(
      children: [
                ...
                builder: (context, state) {
                  if (state is InputserviceLoaded) {
                    ...
                    for (var element in state.itemCards) {
                      total += element.price * (element.pcs);
                    }
                    return Text(
        ...
        Positioned(
          top: 0,
          right: 8,
          child: FloatingButton(formstate: GlobalKey()),
        ),
      ],
    );
  }
...
class FloatingButton extends StatelessWidget {
  ...
  void ontap(BuildContext builcontx) {
    showCupertinoDialog(
  ...
  Widget build(BuildContext context) {
    return FloatingActionButton(
      child: const Column(
        ...
        children: [
          Icon(Icons.upload),
          Text('Submit', textScaler: TextScaler.linear(0.75)),
        ],
      ),
      onPressed: () {
        var clickstate =
        ...
        if (clickstate.itemCards.isNotEmpty) {
          ...
          //     const SnackBar(content: Text('Ada kesalahan pengisian data.')),
          //   );
          //   return;
          // } else {
          ontap(context);
