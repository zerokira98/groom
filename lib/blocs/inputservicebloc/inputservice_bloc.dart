...
class InputserviceBloc extends Bloc<InputserviceEvent, InputserviceState> {
  ...
  InputserviceBloc({
          ...
          Response res = await strukrepo.insertStruk(a).then((value) async {
            var res = await midApi
                ...
                .then((val) {
                  ...
                  if (!(val.headers['content-type']?.contains(
                        "application/json",
                      ) ??
                      true)) {
                    value.delete();
    ...
    on<AddCard>((event, emit) {
      if (state is InputserviceLoaded) {
        var theState = (state as InputserviceLoaded);
        int idx = theState.itemCards.isEmpty
            ? 0
            : theState.itemCards.last.index + 1;
        var newCard = event.data.copyWith(index: () => idx);
        emit(
          (state as InputserviceLoaded).copyWith(
            itemCards: theState.itemCards + [newCard],
          ),
        );
      }
    });
    on<ChangeItemType>((event, emit) {
      ...
      //         .map(
      //           (e) => e.index == event.idx
      //               ? e.copyWith(id: event.id, price: price)
      //               : e,
      //         )
      //         .toList(),
      //   ),
    ...
    on<ChangeTipePembayaran>((event, emit) {
      ...
      if (state is InputserviceLoaded) {
        emit(
          (state as InputserviceLoaded).copyWith(tipePembayaran: event.type),
        );
      }
    });
    on<ChangeKaryawan>((event, emit) {
      if (state is InputserviceLoaded) {
        emit(
          (state as InputserviceLoaded).copyWith(
            karyawanName: event.karyawanName,
          ),
        );
      }
    });
